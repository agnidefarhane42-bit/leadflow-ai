  ...
  try {
    ...
    return NextResponse.json({
      stats: {
        ...
        hot: campaignProspects.filter((p) => (p.score ?? 0) >= 70).length,
        warm: campaignProspects.filter((p) => (p.score ?? 0) >= 40 && (p.score ?? 0) < 70).length,
        cold: campaignProspects.filter((p) => (p.score ?? 0) < 40).length,
